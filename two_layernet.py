...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #softmax
        u= np.exp(scores) 
        scores = u/np.sum(u, axis=1, keepdims=True)
        pass
        ...
        err = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)
        data_loss = np.sum(-np.log(err[range(N), y])) / N
        lambda_1 = 0.05
        loss = data_loss + lambda_1*(np.sum(np.power(W1,2))+np.sum(np.power(W2,2)))
        pass
